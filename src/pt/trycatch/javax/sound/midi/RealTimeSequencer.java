...
class RealTimeSequencer extends AbstractMidiDevice implements Sequencer, AutoConnectSequencer {
	...
	public float getTempoInMPQ() {
        ...
        if (needCaching()) {
            ...
            return MidiUtils.DEFAULT_TEMPO_MPQ;
        }
    ...
    private void doAutoConnect() {
        ...
        try {
            // CHANGED DANGER Synthesizer synth = MidiSystem.getSynthesizer();
        	// CHANGED DANGER synth.open();
        	// CHANGED DANGER if (synth instanceof ReferenceCountingDevice) {
        	// CHANGED DANGER     rec = ((ReferenceCountingDevice) synth).getReceiverReferenceCounting();
        	// CHANGED DANGER } else {
        	// CHANGED DANGER     rec = synth.getReceiver();
        	// CHANGED DANGER }
        } catch (Exception e) {
    ...
    class PlayThread implements Runnable {
        ...
        PlayThread() {
            ...
            thread.setName("Java Sound Sequencer");
            thread.setDaemon(false);
            thread.setPriority(priority);
            thread.start();
            // CHANGED DANGER thread = JSSecurityManager.createThread(this,
         // CHANGED DANGER                                         "Java Sound Sequencer", // name
         // CHANGED DANGER                           false,                  // daemon
         // CHANGED DANGER                            priority,               // priority
         // CHANGED DANGER                           true);                  // doStart
        }
        ...
        private long tick2millis(long tick) {
            if (divisionType != Sequence.PPQ) {
                double dMillis = ((((double) tick) * 1000) /
                                  (tempoFactor * ((double) divisionType) * (resolution)));
                return (long) dMillis;
            }
